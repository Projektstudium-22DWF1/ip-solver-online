...
describe("InputFormatChooser Component", () => {
  ...
  test("renders without crashing and displays correct label", () => {
    render(
      <LanguageContext.Provider value={{ translations: mockTranslations }}>
        <InputFormatChooser inputFormat={InputOptions.LP} setInputFormat={mockSetInputFormat} />
      </LanguageContext.Provider>
    );

    // Überprüfe, ob das Label korrekt angezeigt wird
    expect(screen.getByLabelText(mockTranslations.inputFormatOptions)).toBeInTheDocument();
  });
  ...
  test("renders correct options", () => {
    ...
    const selectElement = screen.getByLabelText(mockTranslations.inputFormatOptions);
  ...
  test("calls setInputFormat when a new option is selected", () => {
    ...
    fireEvent.change(screen.getByLabelText(mockTranslations.inputFormatOptions), {
